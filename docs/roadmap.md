...
| --------------------------------------- | -------------------------------------------------------------------- | --------------------------------------------- | -------- |
| ~~`SortBindings` per execution~~            | ~~`evalBindingsCore` が毎回 `core.SortBindings` を呼ぶ; Runtime は不変~~ | ~~`NewRuntime` で事前計算、ソート済み結果を保持~~ — **done (v0.13)** | ~~High~~     |
| ~~`Env.Lookup` no flattening~~              | ~~深い環境で O(depth) 探索; 設計コメントは flatten を約束~~              | ~~深度閾値で flatten をトリガー~~ — **done (v0.13)**                 | ~~High~~     |
| ~~`UnwindApp` O(n²)~~                       | ~~prepend ベースの引数収集が毎回反転~~                                   | ~~reverse 収集→一度だけ反転~~ — **done (v0.13)**                     | ~~High~~     |
| ~~`PushFront` full copy~~                   | ~~`append(cts, w.items...)` が kickout 毎にワークリスト全体をコピー~~    | ~~Head-index deque or ring buffer~~ — **done (v0.13)**               | ~~Medium~~   |
| ~~`removeClass`/`removeFunEq` linear scan~~ | ~~各 KickOut で線形探索 + スライススプライス~~                           | ~~Pointer identity set or swap-remove~~ — **done (v0.13)**           | ~~Medium~~   |
| ~~`constraintKey` allocation~~              | ~~ホットパスで制約毎に `strings.Builder`~~                               | ~~CtClass 上の lazy key、一度だけ計算~~ — **done (v0.13)**           | ~~Low~~      |
| ~~`isAmbiguousInstance` repeated trial~~    | ~~同一 (class, args) のメモ化なし~~                                      | ~~Per-solve-pass cache~~ — **done (v0.13)**                          | ~~Low~~      |
...
| Item                          | Issue                                                                                 | Priority |
| ----------------------------- | ------------------------------------------------------------------------------------- | -------- |
| ~~Type-level import collision~~   | ~~`importOpen` が Types/Classes/Aliases/Families を `checkAmbiguousName` なしで書き込む~~ — **done (v0.13)**: `importOpen` uses `checkAmbiguousTypeName` for all type-level names | ~~High~~     |
| ~~Private export leak~~           | ~~`ExportModule` が `_` prefix を Values のみフィルタ; types/classes/aliases がリーク~~ — **done (v0.13)** | ~~High~~     |
...
| Item                        | Issue                                               | Fix                                                                           | Priority |
| --------------------------- | --------------------------------------------------- | ----------------------------------------------------------------------------- | -------- |
| ~~Syntax error specialization~~ | ~~E0100 一つに全構文エラーが集約~~                      | ~~E0101–E0110 に分化~~ — **done (v0.13)** | ~~High~~     |
| ~~Expression-level recovery~~   | ~~`case { + -> * }` で 20+ エラー cascade~~             | ~~`ExprError` ノード返却による phrase-level recovery~~ — **done (v0.13)**                            | ~~High~~     |
...
| ~~`main :=` silent acceptance~~ | ~~body なしバインディングがエラーなし~~                 | ~~E0103 MissingBody~~ — **done (v0.13)**                                        | ~~Low~~      |
...
#### ~~`do` Elaboration Consolidation~~ — **done (v0.13)**

~~`inferDo` / `elaborateStmtsChecked` / `elaborateDoMonadic` / `elaborateDoMult` の 4 実装が文処理を重複~~ — `doElaborator` struct で 3 パス (infer/checked/monadic) を統合。`elaborate_do_checked.go` を削除、文 dispatch ループを 1 箇所に集約。
