...
`sh "grep" ["-r", "pattern"]` — 引数は `List String` で構築時に個別に確定する。POSIX シェルの word splitting・glob 展開・クォート解釈は行わない。引数の動的構築（`lines r.#out` で引数リストを作る、`"myapp:" <> version` で文字列を組み立てる等）は自由にできるが、それは GICEL の式評価であり、シェルの文字列評価ではない。
...
```gicel
main := do {
  tool := sh "my-tool" ["--verbose"] & fd @log 3 & redir @log "debug.log" Overwrite;
  run tool
}
```
...
```gicel
main := do {
  tool := sh "my-tool" [] & fd @log 3 & redir @log "debug.log" Overwrite;
  sort := sh "sort" [];
  run $ tool |> sort
}
```
...
```gicel
main := do {
  make := sh "make" ["all"] & mergeErr;
  tee  := sh "tee" ["build.log"];
  run $ make |> tee
}
```
...
main := do {
  set ErrExit;
  build := sh "go" ["build", "-o", "bin/app", "./cmd/app/"] & env "CGO_ENABLED" "0";
  run build
}
```
...
| POSIX                        | 理由                                                                             |
| ---------------------------- | -------------------------------------------------------------------------------- |
| `< file`（入力リダイレクト） | `echo (readFile path)` で代替。stdin は echo + `\|>` で注入する設計              |
| `<< EOF`（heredoc）          | `echo` に複数行文字列を渡す                                                      |
| Glob 展開 (`*.go`)           | シェルの文字列評価を排除する設計方針。`sh "find" [...]` + `lines` で明示的に構築 |
