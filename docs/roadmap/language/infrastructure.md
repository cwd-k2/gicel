...
```
    ...
    └──────────┬──────────┘
               │
    ┌──────────┴──────────┐
    │ L4: Solver          │  L1-c S3 + L2-a full + OutsideIn(X)
    │   coupling (長期)   │
    └─────────────────────┘
```
...
| 項目                      | 状態     | 備考                                                           |
| ------------------------- | -------- | -------------------------------------------------------------- |
| L0-a: Data.Set.map        | 解決済み | v0.18.0 で修正。`map (\x. x*2) (fromList [3,1,2])` → `[2,4,6]` |
| L0-c: Optimizer asymmetry | 解決済み | `postCheck` が module/main 両方で `OptimizeProgram` を呼ぶ     |
| L0-d: Tuple Eq/Ord        | 未着手   | 独立、需要発生時に対応                                         |

## 長期残存

### L1-c Stage 3 / L2-a 完全移行 / L4

GADT given eq の solver 統合、normalize FamilyReducer 除去、OutsideIn(X) 本格導入は相互依存しており、L4 として統合的に扱う。現状は `InstallGivenEq` + eager normalize + deferred CtFunEq の三層で正しく動作。
