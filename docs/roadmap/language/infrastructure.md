...
**クリティカルパス**: L1-b → L1-c → L2-a → L3

## 動機

### 現状の問題構造

1. **Universe stratification 未施行** — `form Foo :: Sort := ...` がコンパイルを通る。Type-in-Type。
...
6. **型等式制約の不在** — `(a ~ Int) =>` がドキュメント (grammar-reference.md, language.md) に記載されているが、パーサーが `TokTilde` を型式中で消費せず、実際には書けない。GADT マッチ由来の given eq は内部生成されるが、ユーザが明示的に等式制約を宣言する経路がない。型族の結果間の関係 (`ElemOf c1 ~ ElemOf c2`) や session type の双対制約 (`SessionDual s1 ~ s2`) が表現できず、型族 + indexed monad の表現力が制限されている。
...
- 型等式制約は、型族の結果を wanted として solver に投入し、meta 解決後の re-activation で reduce する経路を開く。inert set scope (L1-b) と on-demand reduction (L2-a) の自然な消費者。
...
L0-b の設計は以下の後続拡張を**排除しない**形にする。各拡張の詳細は [universe.md](universe.md)。

- **Non-nullary constructor promotion**: `decl_form.go` の promotion ロジックで、nullary チェックをガード条件として分離し、non-nullary promotion (`Just :: KType -> KData{Maybe}`) を追加する際にガード除去のみで済むようにする。L0-b 後に着手可能。
- **Universe polymorphism**: `KSort{Level: int}` の `int` は固定 level。level metavariable (`KSort{Level: LevelExpr}`) への拡張に備え、`KSort` の `Equal` / `String` / `KindSubst` が `Level` フィールドの型変更に対して局所的に閉じていること。L0-b + L1/L2 安定後に着手。
- **Cumulativity**: kind unification を equality-based で実装する。subkinding (`KType ≤ KSort{0}`) は unification とは別の subsumption パスになるため、equality パスとの干渉はない。Universe polymorphism 後に着手。
...
### L1-c: Equality constraints

ユーザが型シグネチャに `(a ~ Int) =>` と書ける構文を導入し、solver に CtEq（型等式 wanted）を投入する経路を開く。

**背景**: grammar-reference.md と language.md に `~` が "Type equality constraint" として記載されているが、実装が存在しない。`TokTilde` はレキサーにあるがパーサーで未消費。GADT マッチ由来の given eq (`extractGivenEqs` → `InstallGivenEq`) は内部生成されるが、ソース構文からの等式制約パスがない。

**表現力**: 型族の結果間の等式が第一級の制約として表現可能になる。

```gicel
-- 型族の結果に名前を付ける
insert :: \a c. (ElemOf c ~ a, Collection c) => a -> c -> c

-- 2つの型族の結果が一致することを制約
transfer :: \c1 c2. (ElemOf c1 ~ ElemOf c2, Collection c1, Collection c2)
  => c1 -> Computation r r c2

-- Session type の双対制約
connect :: \s1 s2. (SessionDual s1 ~ s2)
  => Computation s1 {} a -> Computation s2 {} b -> (a, b)
```
...
| ファイル            | 変更内容                                                              |
| ------------------- | --------------------------------------------------------------------- |
| `syntax/typeast.go` | `TyExprEq` ノード追加: `{Lhs, Rhs TypeExpr; S span.Span}`             |
| `parse/type.go`     | 型式パース中に `TokTilde` を中置演算子として認識し、`TyExprEq` を生成 |
| `syntax/tuple.go`   | `DesugarConstraintTuple` が `TyExprEq` を含む制約タプルを処理         |

`~` の結合力は `=>` より強く、`->` より弱い。制約位置 (`Constraint => Body`) では単独でも制約タプル内でも使える。

**Stage 2: チェッカー + CtEq**

| ファイル                | 変更内容                                                                                                                   |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `solve/constraint.go`   | `CtEq` 型追加: `{Lhs, Rhs types.Type; S span.Span}`                                                                        |
| `solve/solver.go`       | `processCtEq`: 両辺を zonk し `Unify` を呼ぶ。失敗時は型エラー。stuck の場合は inert set に挿入、meta 解決時に re-activate |
| `check/resolve_type.go` | `TyExprEq` の resolve: 両辺を resolve し、CtEq として solver に emit                                                       |
| `check/evidence.go`     | `TyExprQual` の制約位置で `TyExprEq` が来た場合、CtEq を wanted として生成                                                 |

**Stage 3: Given eq 統合**

| ファイル               | 変更内容                                                                                                          |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `check/bidir_case.go`  | GADT ブランチの given eq を CtEq given として solver 経由に統合（現行の `InstallGivenEq` 直接書き込みからの移行） |
| `solve/implication.go` | given CtEq を implication scope 内で正しく管理                                                                    |

Stage 3 は L2-a (on-demand reduction) と連動する。Given eq が solver 管理下に入ることで、given eq → 型族 reduction → 新しい等式の連鎖が自然に動く。

**依存関係**: L1-b (inert set scope) が前提。CtEq が inert set に入るため、scope-aware な管理が必要。Stage 1-2 は L1-b と並行開発可能だが、統合テストは L1-b 完了後。

**検証**:

- `(a ~ Int) => a -> String` が `Int -> String` として使えること
- `(ElemOf c ~ a) =>` で型族の結果を束縛できること
- 制約タプル `(a ~ Int, Eq a) =>` が正しくパース・チェックされること
- GADT マッチの既存テストが全 pass（Stage 3 で given eq 経路を統合後）
- Unsatisfiable な等式 (`(Int ~ Bool) =>`) が型エラーになること
