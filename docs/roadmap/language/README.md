...
```
  ...
  ┌──────────────┐                 ┌──────────────────┐
  │ SMC Phase 2  │                 │ L0-b: Universe   │
  │ >< (内部     │                 │   enforcement    │
  │   row merge) │                 └────────┬─────────┘
  └──────┬───────┘                          │
         │                         ┌────────┴─────────┐
  ┌──────┴───────┐                 │ L1: Solver       │
  │ SMC Phase 3  │                 │   foundation     │
  │ dag          │                 │   + Equality     │
  └──────┬───────┘                 │   constraints    │
         │                         └────────┬─────────┘
         │                         ┌────────┴─────────┐
...
**Infrastructure**: L1-b (inert set scope) → L1-c (equality constraints) → L2-a (TF reduction) → L3 (grade algebra)

L0-b (universe) は L3 の前提だが L1/L2 と並行できるため、クリティカルパス上にない。
