...
var EffectSet Pack = func(e Registrar) error {
	...
	e.RegisterPrim("_msetUnionNamed", withLabelCmpFromHandle(msetUnionImpl))
	e.RegisterPrim("_msetIntersectionNamed", withLabelCmpFromHandle(msetIntersectionImpl))
	e.RegisterPrim("_msetDifferenceNamed", withLabelCmpFromHandle(msetDifferenceImpl))
...
func msetFromListImpl(ctx context.Context, ce eval.CapEnv, args []eval.Value, apply eval.Applier) (eval.Value, eval.CapEnv, error) {
	...
	for {
		...
		if con.Con != "Cons" || len(con.Args) != 2 {
			return nil, ce, errMalformed("msetFromList", "list node", con.Con)
		}
		if err := budget.ChargeAlloc(ctx, costAVLNode); err != nil {
			return nil, ce, err
		}
