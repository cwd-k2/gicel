...
func zipImpl(ctx context.Context, ce eval.CapEnv, args []eval.Value, _ eval.Applier) (eval.Value, eval.CapEnv, error) {
	...
	for {
		...
		pairs = append(pairs, &eval.RecordVal{Fields: map[string]eval.Value{ir.TupleLabel(1): xCon.Args[0], ir.TupleLabel(2): yCon.Args[0]}})
		xs = xCon.Args[1]
		ys = yCon.Args[1]
	}
...
func unzipImpl(ctx context.Context, ce eval.CapEnv, args []eval.Value, _ eval.Applier) (eval.Value, eval.CapEnv, error) {
	...
	for {
		...
		a, ok1 := rec.Fields[ir.TupleLabel(1)]
		b, ok2 := rec.Fields[ir.TupleLabel(2)]
		if !ok1 || !ok2 {
	...
	return &eval.RecordVal{Fields: map[string]eval.Value{ir.TupleLabel(1): buildList(as), ir.TupleLabel(2): buildList(bs)}}, ce, nil
}
