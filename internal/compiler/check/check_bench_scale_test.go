...
func doBlockSource(n int) string {
	...
	b.WriteString("form Trivial := { Triv: Trivial }\n")
	b.WriteString("type TrivialJoin :: Trivial -> Trivial -> Trivial := \\(a: Trivial) (b: Trivial). Triv\n")
	b.WriteString("type TrivialCompose :: Trivial -> Trivial -> Trivial := \\(a: Trivial) (b: Trivial). Triv\n")
	b.WriteString("form GradeAlgebra := \\(g: Kind). { type GradeJoin :: g -> g -> g; type GradeCompose :: g -> g -> g; type GradeDrop :: g; type GradeUnit :: g }\n")
	b.WriteString("impl GradeAlgebra Trivial := { type GradeJoin := TrivialJoin; type GradeCompose := TrivialCompose; type GradeDrop := Triv; type GradeUnit := Triv }\n")
	b.WriteString("form GIMonad := \\(g: Kind) (m: g -> Row -> Row -> Type -> Type). GradeAlgebra g => {\n")
	b.WriteString("  gipure: \\a (r: Row). a -> m GradeDrop r r a;\n")
	b.WriteString("  gibind: \\a b (e1: g) (e2: g) (r1: Row) (r2: Row) (r3: Row). m e1 r1 r2 a -> (a -> m e2 r2 r3 b) -> m (GradeCompose e1 e2) r1 r3 b\n}\n")
	b.WriteString("form MyComp := \\(g: Trivial) (s: Row) (t: Row) (a: Type). { MkComp: (Bool -> (a, Bool)) -> MyComp g s t a }\n")
	b.WriteString("impl GIMonad Trivial MyComp := {\n")
	b.WriteString("  gipure := \\a. MkComp (\\s. (a, s));\n")
