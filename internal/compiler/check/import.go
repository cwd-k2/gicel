...
func (sc *Scope) ImportModules(imports []syntax.DeclImport) {
	if sc.config.ImportedModules == nil {
		if len(imports) > 0 {
			sc.session.addCodedError(diagnostic.ErrImport, imports[0].S, fmt.Sprintf("unknown module: %s", imports[0].ModuleName))
		}
		return
	}
	...
	for _, imp := range imports {
		...
		if imp.ModuleName == "Core" && (imp.Alias != "" || imp.Names != nil) {
			sc.session.addCodedError(diagnostic.ErrImport, imp.S,
...
func (sc *Scope) checkAmbiguousName(name, moduleName string, s span.Span) bool {
	...
	sc.session.addCodedError(diagnostic.ErrImport, s,
