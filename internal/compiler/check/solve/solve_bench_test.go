			...
			for i := 0; i < b.N; i++ {
				...
				for j := range n {
					is.InsertFunEq(&CtFunEq{
						FamilyName: "F",
						Args:       []types.Type{meta, testOps.Con("Int")},
...
func BenchmarkInertSetScopeEnterLeave(b *testing.B) {
	for b.Loop() {
		is := &InertSet{}
		is.InsertFunEq(&CtFunEq{FamilyName: "F", Args: []types.Type{testOps.Con("Int")}, ResultMeta: &types.TyMeta{ID: 1, Kind: types.TypeOfTypes}, BlockingOn: []int{1}})
		is.EnterScope()
		for j := range 20 {
			is.InsertFunEq(&CtFunEq{FamilyName: "G", Args: []types.Type{testOps.Con("Int")}, ResultMeta: &types.TyMeta{ID: 100 + j, Kind: types.TypeOfTypes}, BlockingOn: []int{100 + j}})
		}
		is.LeaveScope()
	}
}
		...
		b.Run(sizeName(n), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				for j := range n {
					...
					is.InsertEq(&CtEq{
						Lhs:    meta,
						Rhs:    &types.TyApp{Fun: testOps.Con("F"), Arg: skolem},
						Flavor: CtWanted,
						S:      span.Span{},
					}, []int{j})
				}
