	...
	if p.peek().Kind == syn.TokFatArrow {
		...
		for {
			...
			for _, arg := range nextArgs {
				...
				if !ok {
					p.addErrorCode(diagnostic.ErrClassSyntax, "class type parameter must be a type variable")
					continue
				}
...
func (p *Parser) parseFunDepList() []syn.FunDep {
	...
	for {
		...
		if len(to) == 0 {
			p.addErrorCode(diagnostic.ErrClassSyntax, "functional dependency requires at least one determined parameter after '=:'")
		}
