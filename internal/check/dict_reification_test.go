...
useDict := \d x y -> case d of { MkDict -> eq x y }
...
useEq := \d x y -> case d of { MkDict -> eq x y }
useShow :: Dict (Show Bool) -> Bool -> Unit
useShow := \d x -> case d of { MkDict -> show x }`
	checkSource(t, source, nil)
}
...
useOrd := \d x y -> case d of { MkDict -> eq x y }`
	checkSource(t, source, nil)
}
...
useEvidence := \e -> case e of { MkEvidence x -> eq x True }`
	checkSource(t, source, nil)
}
...
chain := \d1 d2 x -> case d1 of { MkDict -> case d2 of { MkDict -> show x } }`
	checkSource(t, source, nil)
}
...
instance Eq Bool { eq := \x y -> True }
data Dict (c : Constraint) = MkDict c
useInferred := case (MkDict :: Dict (Eq Bool)) of { MkDict -> eq True False }`
	checkSource(t, source, nil)
}
