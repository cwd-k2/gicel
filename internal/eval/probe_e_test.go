...
func TestProbeE_AllocLimitExactBoundary(t *testing.T) {
	...
	_, err = ev.Eval(EmptyEnv(), EmptyCapEnv(), &core.Con{Name: "B"})
	if err == nil {
...
func TestProbeE_BounceValNeverEscapes(t *testing.T) {
	...
	var term core.Core = &core.Lit{Value: int64(42)}
	for range 10 {
		term = &core.Case{
			Scrutinee: &core.Con{Name: "True"},
			Alts: []core.Alt{
				{Pattern: &core.PCon{Con: "True"}, Body: term},
			},
		}
	}
...
func TestProbeE_BounceValFromForce(t *testing.T) {
	...
	ev := newTestEval()
	term := &core.Force{
		Expr: &core.Thunk{Comp: &core.Lit{Value: int64(77)}},
	}
	r, err := ev.Eval(EmptyEnv(), EmptyCapEnv(), term)
	if err != nil {
		t.Fatal(err)
...
func TestProbeE_ApplyResolvedNoBounce(t *testing.T) {
	...
	ev := newTestEval()
	clo := &Closure{Env: EmptyEnv(), Param: "x", Body: &core.Var{Name: "x"}}
	r, err := ev.applyResolved(EmptyCapEnv(), clo, &HostVal{Inner: int64(42)}, &core.App{})
	if err != nil {
		t.Fatal(err)
	}
	if _, ok := r.Value.(*bounceVal); ok {
...
func TestProbeE_MatchNestedCon(t *testing.T) {
	...
	pat := &core.PCon{Con: "Just", Args: []core.Pattern{
		&core.PCon{Con: "Cons", Args: []core.Pattern{
			&core.PVar{Name: "x"},
			&core.PWild{},
		}},
	}}
...
func TestProbeE_ValueStringMethods(t *testing.T) {
	...
	tests := []struct {
		...
		{"Closure", &Closure{Env: EmptyEnv(), Param: "x", Body: &core.Var{Name: "x"}}},
		{"ThunkVal", &ThunkVal{Env: EmptyEnv(), Comp: &core.Lit{Value: int64(0)}}},
		...
		{"bounceVal", &bounceVal{env: EmptyEnv(), capEnv: EmptyCapEnv(), expr: &core.Lit{Value: int64(0)}}},
	}
...
func TestProbeE_BindDepthUnwind(t *testing.T) {
	...
	term := &core.Bind{
		Comp: &core.Pure{Expr: &core.Lit{Value: int64(1)}},
		Var:  "_",
		Body: &core.Pure{Expr: &core.Lit{Value: int64(2)}},
	}
	_, err := ev.Eval(EmptyEnv(), EmptyCapEnv(), term)
	if err != nil {
		t.Fatal(err)
...
func TestProbeE_RecordUpdateAddsNewField(t *testing.T) {
	...
	term := &core.RecordUpdate{
		...
		Updates: []core.RecordField{
			{Label: "y", Value: &core.Lit{Value: int64(2)}},
		},
	}
	r, err := ev.Eval(EmptyEnv(), EmptyCapEnv(), term)
	if err != nil {
...
func TestProbeE_CapEnvThreadingThroughConArgs(t *testing.T) {
	...
	term := &core.Con{
		Name: "Pair",
		Args: []core.Core{
			&core.PrimOp{Name: "markCap", Arity: 0, Args: nil},
